...
function renderList() {
  ...
  api.getAll().then((tasks) => {
   ...
   if (tasks && tasks.length > 0) {
      ...
      tasks.sort((a, b) => new Date(a.completed).getTime() - new Date(b.completed).getTime());
...
function renderTask({ id, title, description, dueDate, completed }) {
  ...
  if (completed) {
    html = `
    <li id="${id}" class="select-none mt-2 bg-green-200 py-2 border-b border-amber-800">
      <div class="flex items-center">
      <input type="checkbox" onclick="updateTask(${id})"  checked  class="mr-4 ">
      <h3 class="mb-3 flex-1 text-xl font-bold text-gray-400 line-through uppercase">${title}</h3>
        <div>
          <span>${dueDate}</span>
